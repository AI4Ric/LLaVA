...
class ModelWorker:
    ...
    def generate_text(self, params):
        ...
        # Directly call the model's generate function
    ...
    def generate_text_gate(self, params):
        try:
            result = self.generate_text(params)
            return json.dumps(result).encode() + b"\0"
...
async def generate_text(request: Request):
    ...
    # Get the full response from the modified function
    response = worker.generate(params)

    # Release the semaphore and send a heart beat in the background
    background_tasks = BackgroundTasks()
    background_tasks.add_task(partial(release_model_semaphore, fn=worker.send_heart_beat))
